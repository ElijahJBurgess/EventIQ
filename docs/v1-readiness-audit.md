...
**Method:** The audit pass itself was discovery + documentation only. Every "broken" / "working" claim is backed by an actual test; the verification method is stated per finding. Fixes made *after* the audit, each under its own explicit approval, are logged in [Appendix B](#appendix-b--changes-made).

**Fix passes since the audit:**
- **2026-09-04 — `admin-gen-link` decommissioned** (finding S3). See §2.5 / Appendix B.
- **2026-09-04 — Group 1 security fix pass** (findings S1, S2, S5, and the CORS half of S4). Applied via migration `20260904010000_group1_security_lockdown.sql` + `admin-auth` v22; each fix live-re-verified the same way the audit found it. See per-finding notes below and Appendix B. **S6 (SECURITY DEFINER views) was explicitly deferred to its own pass** — plan documented in §3.3.
...
| # | Finding | §|
|---|---|---|
| S1 | ~~`reports` table is **world-readable, including by anonymous users** (`SELECT` policy `true` for PUBLIC).~~ **RESOLVED 2026-09-04 (Group 1)** — dropped the `"Anyone can view reports"` policy and revoked `SELECT` from `anon` + `authenticated`; `reports` is now service-role-only (no client path reads it directly). Live-verified: anon → `401 42501 permission denied`, signed-in user → `403 42501`; `admin-auth` `list-reports` still returns rows. | 3.2 / 3.4 |
| S2 | ~~`feedback` table is **readable by every authenticated user** (`SELECT true`).~~ **RESOLVED 2026-09-04 (Group 1)** — SELECT policy is now `USING ((select auth.uid()) = user_id)` (author-only; no organizer carve-out — the dashboard reads `feedback` via the service role). Live-verified: test user B reading test user A's row → `[]`; A reading its own row → returns it; A's INSERT path still `201`. | 3.2 / 3.4 |
...
| S4 | `admin-auth` posture. **CORS half RESOLVED 2026-09-04 (Group 1)** — the wildcard `Access-Control-Allow-Origin: *` is replaced with an explicit allow-list (`https://event-iq-six.vercel.app` + `localhost:8080`/`127.0.0.1:8080`, extensible via `ADMIN_AUTH_ALLOWED_ORIGINS`); a browser request from a disallowed origin now gets `403` with no `ACAO`. Live-verified (foreign origin blocked; prod + localhost echoed; no-Origin non-browser calls unaffected). **Still open:** single static shared password with no rotation/rate-limit/lockout, and the catch-all that returns `{valid:false}` for downstream errors as for auth failure. | 2.2 / 3.5 |
| S5 | ~~`profiles` UPDATE policy has **no `WITH CHECK`** → `UPDATE profiles SET id='<other>'` isn't blocked by RLS.~~ **RESOLVED 2026-09-04 (Group 1)** — policy recreated with `WITH CHECK ((select auth.uid()) = id)` (and narrowed `TO authenticated`). Live-verified: a signed-in user re-targeting its own row's `id` → `403 new row violates row-level security policy`; a normal self-update → `200`. | 3.2 |
| S6 | `attendee_profiles` + `matched_event_attendance` are **`SECURITY DEFINER` views** (Supabase advisor: ERROR). **DEFERRED 2026-09-04 to its own pass** (excluded from Group 1 by decision — it's a core read-path refactor across ~13 call sites + `concierge`, not a contained RLS change). Investigation + fix plan (replace the views with `SECURITY DEFINER` *functions*, keeping the curated column subset as the boundary) documented in §3.3. A plain `security_invoker` flip is **not** viable — it breaks matched-profile viewing app-wide. | 3.3 |
| S7 | **Email verification is disabled** in prod (`enable_confirmations = false`). | 3.6 / 5.4 |
| S8 | ~~`EditProfileScreen` **cannot save** a profile whose `location_preference` is `NULL` unless the user picks a value — writes `""` → check-constraint violation → generic "save failed".~~ **RESOLVED 2026-09-08** — both the editor and the onboarding wizard now build their `profiles` update through a shared `buildProfileUpdatePayload()` that coerces the 7 "pick one" fields (`role_type`, `primary_function`, `seniority`, `primary_goal`, `matching_goal`, `industry_preference`, `location_preference`) from `""` → `null`. Live-verified: the old `""` payload → `400 23514`; the new `null` payload → `204` (saves cleanly, prefs stay null); valid values still save. **Tradeoff (accepted):** a hard failure becomes a silent success — a user can now save a profile with these fields blank, and `profile_completed` isn't recomputed by the editor. The follow-up is inline "required" validation in the editor (not done). | 1.3 |
...
- ✅ ~~**`EditProfileScreen` can't save a profile whose `location_preference` is `NULL`.**~~ **RESOLVED 2026-09-08 (commit follows).** `profiles_location_preference_check` allows only `NULL` or `{prioritize_city, prioritize_outside_city, mix, no_preference}`; the editor loaded `?? ""` and wrote `""` back → `23514`. Same shape affected `role_type` / `primary_function` / `seniority` (their `x = ANY(...)` CHECKs pass on `NULL` but reject `""`) and silently-junk `primary_goal` / `matching_goal` / `industry_preference`. **Fix:** extracted the shared `.update()` payload (used by both `EditProfileScreen.saveChanges` and `ProfileSetup.onSubmit`) into `src/components/profile-setup/profileUpdate.ts` `buildProfileUpdatePayload()`, which `""` → `null` coerces those 7 fields; free-text fields pass through unchanged. TDD (`profileUpdate.test.ts`, 5 cases). Live-verified against a throwaway profile with NULL prefs: old payload `400 23514`, new payload `204`, valid values `204`. `ProfileSetup` was a no-op change (its fields are always populated) but is now hardened + de-duplicated. **Tradeoff:** the hard failure becomes a silent success (profile can be saved incomplete; editor doesn't recompute `profile_completed`). Follow-up = inline required-field validation in the editor.
...
- ✅ ~~**`Access-Control-Allow-Origin: *`.**~~ **RESOLVED 2026-09-04 (Group 1), `admin-auth` v22.** Replaced the wildcard with the same origin-allow-list pattern the other functions use: `https://event-iq-six.vercel.app` + `http://localhost:8080` + `http://127.0.0.1:8080`, plus anything in the optional `ADMIN_AUTH_ALLOWED_ORIGINS` secret. A browser request from a disallowed origin now short-circuits to `403` with **no** `Access-Control-Allow-Origin` header. Non-browser callers (no `Origin` header) are unchanged. Live-verified: `evil.example.com` OPTIONS+POST → `403`, no ACAO; `event-iq-six.vercel.app` and `localhost:8080` → `200` with the origin echoed + `Vary: Origin`; a no-Origin `event-stats` call → `200` with the full payload.
...
|---|---|---|
| `profiles` | SELECT/INSERT own only; other profiles via the `attendee_profiles` view | ✅ **RESOLVED 2026-09-04 (Group 1)** — UPDATE policy recreated with `WITH CHECK ((select auth.uid()) = id)` and narrowed `TO authenticated`. Live-verified: a signed-in user trying to re-target its own row's `id` → `403 new row violates row-level security policy`; a normal self-update → `200`; targeting another user's row → 0 rows (unchanged). |
| `feedback` | INSERT `auth.uid()=user_id` ✅ | ✅ **RESOLVED 2026-09-04 (Group 1)** — SELECT policy replaced with `USING ((select auth.uid()) = user_id)` (author-only). No organizer carve-out: there is no real per-user organizer role, and the dashboard reads `feedback` via the service role. Live-verified: cross-user read → `[]`; own read → row returned; own INSERT → `201`; INSERT as another `user_id` → `403`. |
| `reports` | INSERT `auth.uid()=generated_by` | ✅ **SELECT RESOLVED 2026-09-04 (Group 1)** — dropped the PUBLIC `"Anyone can view reports"` policy and `REVOKE SELECT ... FROM anon, authenticated`; `reports` is service-role-only now (no client reads it directly; `admin-auth` uses the service key). Live-verified: anon → `401 42501`, signed-in → `403 42501`, `admin-auth` `list-reports` still returns rows. **Still open (S9, not in Group 1):** INSERT isn't organizer-checked — any signed-in user can create `reports` rows with an attacker-controlled `raw_metrics` jsonb. |
...
- 🟠 **`attendee_profiles` and `matched_event_attendance` are `SECURITY DEFINER` views** (advisor level **ERROR**) — **DEFERRED 2026-09-04 to its own pass** (excluded from Group 1 by decision).
  - *Why they're DEFINER — investigated:* migrations `20260821010000` / `20260821030000` were a deliberate hardening: they revoked broad `profiles` / `event_registrations` SELECT, locked the base tables to own-row-only, and created these `security_invoker = false` views to expose a **curated safe column subset** (no `email`, no personal URLs, no completion/auth fields) to *matched* counterparts. The DEFINER mode is what lets a user see a match's networking fields their own RLS forbids. Not a workaround — an intentional design.
  - *Why a plain `security_invoker = true` flip is NOT viable:* under invoker mode the view runs with the caller's own-row-only RLS → the `EXISTS(shared match)` branch returns nothing → **matched-profile viewing and matched check-in status break across the whole app** (11 `attendee_profiles` call sites in `MatchesTab` / `MessagesTab` / `NotificationBell` / `ConnectionSelfReportPrompt` / `matchDetail` / `Dashboard` ×6, plus `concierge/context.ts` which runs as the caller).
  - *Planned fix (its own pass):* replace both views with `SECURITY DEFINER` **functions** (`SET search_path = ''`, `STABLE`, `GRANT EXECUTE TO authenticated`) returning the same safe columns with the same `WHERE`. This is Supabase's recommended replacement for a DEFINER view — it clears the `0010` ERROR, keeps the column subset as the boundary, and trades the 2 ERRORs for 2 `0029` WARNs (the same already-accepted class as the 8 meeting/connection RPCs). Cost: ~13 client call sites move from `.from(view)` to `.rpc()`, plus a `concierge` redeploy, `types.ts` regen, and touched tests.
  - Live-verified (during the audit) the current WHERE clause contains correctly — a test user saw only their own profile + shared-match profiles, and `email` / `linkedin_url` are not in the view's column list — so this is a lint-compliance + brittleness fix, not an active leak.
...
| ~~Read all `feedback`~~ → **re-probed 2026-09-04 after Group 1** | cross-user read → `[]` ✅ (own rows still readable) |
| ~~Read all `reports` with no auth (anon)~~ → **re-probed 2026-09-04 after Group 1** | anon → `401 42501`; signed-in → `403 42501` ✅ |
| ~~`profiles` UPDATE with no `WITH CHECK`~~ → **re-probed 2026-09-04 after Group 1** | id re-target → `403 new row violates RLS policy` ✅ |
| Anon read `profiles` | `42501` ✅ |

**Verdict (updated 2026-09-04):** the core relationship data (profiles, matches, messages, meetings, notifications, connection notes/reports) is correctly walled off per-user, and the write paths are all funnelled through guarded RPCs. The Group 1 fixes closed the `feedback` over-share, the `reports` public leak, and the `profiles` UPDATE `WITH CHECK` gap — all re-verified live. The remaining boundary item is the SECURITY DEFINER views (S6), deferred to its own pass.
...
| `admin-auth` | ~~**`*`**~~ → **RESOLVED 2026-09-04 (Group 1), v22:** origin allow-list (`event-iq-six.vercel.app` + `localhost:8080`/`127.0.0.1:8080` + `ADMIN_AUTH_ALLOWED_ORIGINS`); disallowed origin → `403`, no ACAO | ✅ live-verified |
| `admin-gen-link` | no CORS headers (decommissioned — §2.5) | n/a |
...
| **Group 1 security fix pass** (findings S1, S2, S5, CORS half of S4) | Migration `20260904010000_group1_security_lockdown.sql` + `admin-auth` v22. **S1** `reports`: dropped `"Anyone can view reports"` policy, `REVOKE SELECT FROM anon, authenticated` → service-role-only. **S2** `feedback`: SELECT policy → `USING ((select auth.uid()) = user_id)` (author-only). **S5** `profiles`: UPDATE policy recreated with `WITH CHECK ((select auth.uid()) = id)`, `TO authenticated`. **S4 (CORS)** `admin-auth`: wildcard ACAO → origin allow-list (`event-iq-six.vercel.app`, `localhost:8080`, `127.0.0.1:8080`, `ADMIN_AUTH_ALLOWED_ORIGINS`); disallowed origin → `403` no ACAO. | Done. `tsc -b` 0, 35 files / 221 tests, lint 0 errors (8 pre-existing warnings), `npm run build` 0. Each fix live-re-verified against prod (see per-finding notes) + flow re-checks: Reports tab (`list-reports` → rows), feedback submit (`201` + own read), profile edit (normal self-update `200`), `/v2/admin` (`event-stats` `200` full payload from prod origin). Throwaway `audit-20260904-a/-b` accounts + one unpublished test event: all deleted, baseline restored (0 leftovers). |
| Deferred: SECURITY DEFINER views (finding S6) | Excluded from Group 1 by decision — it's a core read-path refactor (~13 call sites + `concierge`), not a contained RLS change. Investigation + fix plan (views → `SECURITY DEFINER` functions) documented in §3.3. | Open — its own pass. |
